...
function getElement(x, y){
	...
	for(var i = edges.length - 1; i >= 0; i--){
		// Compute final_dist, the distance from the mouse at (x,y) to the edge e
		var e       = edges[i]
		var ax      = e.from.x
		var ay      = e.from.y
		var bx      = e.to.x
		...
		var r_nume  = (x-ax)*(bx-ax) + (y-ay)*(by-ay)
		var r_denom = (bx-ax)*(bx-ax) + (by-ay)*(by-ay)
		var r       = r_nume/r_denom
		
		var px      = ax + r*(bx-ax)
		var py      = ay + r*(by-ay)
		var s       = ((ay-y)*(bx-ax)-(ax-x)*(by-ay) ) / r_denom
		var tol     = (touch ? 15 : 6)
		
		var distance_to_line = Math.abs(s) * Math.sqrt(r_denom)
		var final_dist
		if(r >= 0 && r <= 1){
			final_dist = distance_to_line
		} else {
			var dist1 = (x-ax)*(x-ax) + (y-ay)*(y-ay)
			var dist2 = (x-bx)*(x-bx) + (y-by)*(y-by)
       
			if(dist1 < dist2){
				final_dist = Math.sqrt(dist1)
			} else {
				final_dist = Math.sqrt(dist2)
			}
		}
		if(final_dist <= tol){
			return e
